...
> ⚠️ **SUPERSEDED & ARCHIVED (2026-05-27).** Absorbed into the Unified Context Pipeline (Option C). The `load: always | by-request` field + its filtering now land in `context-pipeline-plan-002` (Phase 2); the VS Code tree-view UX (📌 icon) rides with the Phase 3 sidebar CONTEXT work. Authoritative record: `context-pipeline-design.md` §14. This design predates the assembler and was written for the old `getAIContext` path that no longer exists — kept for provenance only, not a live design.
