...
export async function promoteToPlanCommand(treeProvider: LoomTreeProvider, node?: TreeNode): Promise<void> {
    ...
    if (await isClaudeInstalled()) {
        ...
        await launchClaude(root, `Loom: Promote to Plan`,
            `Loom promote to plan task. sourceId="${sourceId}", targetWeaveId="${targetWeaveId}"${threadArg}. Use the loom MCP server: use MCP tool loom_find_doc with id="${sourceId}" to read the source doc, use MCP tool loom_create_plan with weaveId="${targetWeaveId}"${targetThreadId ? ` threadId="${targetThreadId}"` : ''}, then use MCP tool loom_update_doc with a plan steps table derived from the source. Do not use loom_promote — sampling is unavailable in Claude Code CLI.`
        );
    } else {
        try {
            let result: any;
